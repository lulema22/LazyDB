...
//собрать строку обратно из ячеек, тоже через ;
...
public:
    ...
    CsvTablePanel(wxWindow* parent, TableSpec spec) //одна вкладка
        ...
        //выключена, так как включается только после проверки
        saveAsBtn_->Enable(false);


        // ширина колонок, строк и тд
    ...
    wxGrid* GetGrid() const {return grid_;} //подсветить строки //возвращает указатель на таблицу
    const TableSpec& GetSpec() const {return spec_;} //описание таблицы
    wxString GetPath() const {return picker_->GetPath();} //возвращает путь к файлу
    wxString GetDefaultFilename() const {return spec_.defaultFilename; } //дефолтное имя файла

    void EnableSaveAs(bool en) {if (saveAsBtn_) saveAsBtn_->Enable(en); } //активность кнопки

    void SetDefaultPath(const wxString& fullPath) {
        picker_->SetPath(fullPath); //путь по умолчанию
    }
    ...
    void LoadFromFile(const wxString& path) {
        ...
        while (std::getline(in, line)) {
            ...
            if ((int)parts.size() != (int)spec_.columns.size()) { //проверка на число колонок
...
public:
    ...
    bool IsValidatedOk() const { return validatedOk_; }
...
private:
    ...
    void UpdateSaveButtons() {
        ...
        for (auto* p : panels_) {
            if (p) p->EnableSaveAs(validatedOk_);
        }
    }
    ...
    static wxString EnsureTempDir() { //возвращает системную временную папку:
    ...
    static wxString WriteTempCsv(const wxString& dir, const wxString& name, const std::vector<std::string>& lines) {
        ...
        for (const auto& l : lines) out << l << "\n";
        return path; //функция делает физический временный файл с текущим содержимым гридов.
    }
    ...
    bool RowAllEmpty(wxGrid* g, int r) const {//проверка строка полностью пустая
        if (!g) return true;
    ...
    void RequireNonEmptyFieldsOrThrow(int tab, const std::vector<int>& requiredCols) { //tab это номер вкладки (0..5)// requiredCols список индексов колонок, которые обязательны
        ...
        for (int r = 0; r < g->GetNumberRows(); ++r) {
            ...
            for (int col : requiredCols) {
                ...
                if (g->GetCellValue(r, col).IsEmpty()) {
                    notebook_->SetSelection(tab); //переключает пользователя на вкладку с индексом tab
                    ClearLastHighlight(); //убирает предыдущую подсветку
                    lastTab_ = tab; //запоминаем: в какой вкладке мы подсветили ошибку
                    lastRow_ = r;
                    lastCol_ = col;
                    lastOldColor_ = g->GetCellBackgroundColour(r, col); //сохр фон
                    g->MakeCellVisible(r, col); //прокрутка
                    g->SetGridCursor(r, col);
                    g->SelectBlock(r, col, r, col); //выделяем ячейку
                    g->SetCellBackgroundColour(r, col, wxColour(255, 220, 220));
                    g->ForceRefresh();
                    throw std::runtime_error(
                        "Required cell is empty: table=" +
                        panels_[tab]->GetSpec().title.ToStdString() + " row=" + std::to_string(r) + " col=" + panels_[tab]->GetSpec().columns[col].ToStdString()
                    );
                }
            }
        }
    ...
    Database BuildDbFromGridsOrThrow() {
        ...
        const wxString dir = EnsureTempDir(); //создаем tmp директорию
        //пишем для каждой таблицы
        ...
        return Database::LoadFromFiles( addrPath.ToStdString(),deptPath.ToStdString(),empPath.ToStdString(),
            supPath.ToStdString(),prodPath.ToStdString(),purPath.ToStdString()
        );
    }

    static int GetSelectedRow(wxGrid* g) { //выделение строки (если ничего - то где курсор) если несколько то первую
        if (!g) return -1;
    ...
    static int GetIdFromRow(wxGrid* g, int row) { //достать id из первой колонки
    ...
    int GetFirstIdInTab(int tab) const { // хоть какой-то сущ id, для проверки fk
        wxGrid* g = panels_[tab]->GetGrid();
        if (!g) return -1;
        for (int r = 0; r < g->GetNumberRows(); ++r) {
            if (RowAllEmpty(g, r)) continue;
    ...
    void OnDeleteSelected(wxCommandEvent&) {
        ...
        try {
            Database db = BuildDbFromGridsOrThrow(); // строим файл временный
            switch (tab) {
            ...
            validatedOk_ = false;
            ClearLastHighlight();
            UpdateSaveButtons(); //кнопки сохранения отклюяаются

            AppendLog(wxString::Format("Deleted id=%d from tab=%d", id, tab));

        } catch (const DbConstraintError& e) { //ловим исключение по ссылке
            validatedOk_ = false;
            UpdateSaveButtons();
            AppendLog(wxString::Format("Constraint error: %s", e.what()));
            HighlightErrorCell(e); //где ошибка (строка поле и тд)
            wxMessageBox(e.what(), "Delete blocked", wxOK | wxICON_ERROR, this);

        } catch (const std::exception& e) { //стандартные ошибки
            validatedOk_ = false;
            UpdateSaveButtons();
    ...
    void OnValidate(wxCommandEvent&) {
        try {
            ...
            UpdateSaveButtons();
            AppendLog(wxString::Format("Constraint error: %s", e.what()));
            HighlightErrorCell(e); //подсвечиваем
    ...
    void OnSaveAllAs(wxCommandEvent&) {
        CommitAllEdits();
        if (!validatedOk_) { //проверка на валид
            wxMessageBox("Please run Validate first (and fix errors) before saving.",
                         "Save All As...", wxOK | wxICON_WARNING, this);
            return;
        }
        wxDirDialog dlg(this, "Choose folder to save ALL CSV files", "", //диалоговое окно выбора папки
...
private:
    ...
    enum class SearchKind { Disabled, ExactString, ExactInt, RangeInt, RangeString }; // типы поиска чисто строка/цифра и тд
    ...
    void SetRangeVisible(bool visible) { //для второго поля поиска
    ...
    void UpdateSearchUIForTab(int tab) { //переключение таблицы
...
void HighlightRowsByIds(int tab, const std::vector<int>& ids) { //подсвечиваем нужные строки
        if (tab < 0 || tab >= (int)panels_.size()) return;
    ...
    void OnClearSearch(wxCommandEvent&) { //снимам выделение строк
        int tab = notebook_->GetSelection();
    ...
    void OnSearch(wxCommandEvent&) {
        try {
            const int tab = notebook_->GetSelection();// получ данные
            const wxString field = searchField_ ? searchField_->GetStringSelection() : "";
            const SearchKind kind = GetSearchKind(tab, field);
            ...
            Database db = BuildDbFromGridsOrThrow(); // строим database LoadFromFiles - BuildIndexes
...
void CsvTablePanel::OnSaveAs(wxCommandEvent&) {
    auto* top = wxGetTopLevelParent(this);
    auto* frame = dynamic_cast<MainFrame*>(top);
    if (frame && !frame->IsValidatedOk()) {
        wxMessageBox("Please run Validate first (and fix errors) before saving.",
                     "Save As...", wxOK | wxICON_WARNING, this);
        return;
    }

    wxFileDialog saveDlg(this, "Save CSV As", "", spec_.defaultFilename,"CSV files (*.csv)|*.csv",wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
